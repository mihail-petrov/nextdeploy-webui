...
export default Ember.Component.extend({
  ...
  didReceiveAttrs() {
    var ep_empty = Ember.Object.create({prefix: '', path: '', aliases: '', envvars: '', framework: null, port: 8080, ipfilter: ''});
  ...
  resetIsInstall: function() {
    if (this.get('project.id')) {
      this.set('endpoint.is_install', false);
    } else {
      this.set('endpoint.is_install', true);
    }
  ...
  actions: {
    ...
    addEndpoint: function() {
      ...
      endpoint = this.store.createRecord('endpoint', { project: this.get('project'), framework: ep.get('framework'), prefix: ep.get('prefix'), path: ep.get('path'), envvars: ep.get('envvars'), aliases: ep.get('aliases') });

      this.get('project').get('endpoints').addObject(endpoint);
      this.set('endpoint', Ember.Object.create({prefix: '', path: '', aliases: '', envvars: '', framework: null}));
    },
